...
################################################################################
VPATH = src:cuda
OPENCV_DEPS = pkg-config --libs --cflags opencv
...
mobileNet: main.cpp util.o tensorNet.o
	$(EXEC) $(NVCC) $(ALL_LDFLAGS) $(GENCODE_FLAGS) -o $@ $+ $(LIBRARIES) `$(OPENCV_DEPS)`
...
clean:
	rm -f mobileNet *.o
	rm -rf bin/$(TARGET_ARCH)/$(TARGET_OS)/$(BUILD_TYPE)/mobileNet
